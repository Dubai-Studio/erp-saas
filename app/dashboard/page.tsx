...
function CTooltip({ active, payload, label }: {
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: 12, padding: '10px 14px', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontSize: 12, minWidth: 180 }}>
...
// ─── KPI Card animée ──────────────────────────────────────────────────────────
function KpiCard({ label, value, sub, color, icon, trend, alert, prefix = '' }: {
  label: string; value: number; sub?: string; color: string
  icon: React.ReactNode; trend?: number; alert?: boolean; prefix?: string
}) {
  ...
  return (
    <div style={{
      ...
      onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-2px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = `0 8px 24px ${displayColor}20` }}
      onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = ''; (e.currentTarget as HTMLDivElement).style.boxShadow = `0 2px 12px ${displayColor}12` }}
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <p style={{ fontSize: 10, fontWeight: 700, color: C.slate, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{label}</p>
        <div style={{ width: 38, height: 38, borderRadius: 10, background: (alert || isNeg ? '#fef2f2' : color + '15'), display: 'flex', alignItems: 'center', justifyContent: 'center', color: displayColor, flexShrink: 0 }}>
          {icon}
        </div>
      </div>
      <div>
        <p style={{ fontSize: 24, fontWeight: 900, color: displayColor, lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
          {isNeg ? '-' : prefix}{fmt(Math.abs(animated)).replace('€', '').trim()} €
        </p>
        {sub && <p style={{ fontSize: 11, color: C.slate, marginTop: 5, lineHeight: 1.4 }}>{sub}</p>}
      </div>
      {trend !== undefined && (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px', borderRadius: 20, background: trend >= 0 ? '#dcfce7' : '#fee2e2', color: trend >= 0 ? '#16a34a' : '#dc2626', fontSize: 11, fontWeight: 700, width: 'fit-content' }}>
...
function StatCard({ label, value, sub, color, icon }: {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 13, border: `1.5px solid ${color}22`, padding: '13px 15px', display: 'flex', alignItems: 'center', gap: 12,
      transition: 'transform 0.2s', cursor: 'default' }}
      onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-2px)' }}
      onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = '' }}
    >
      <div style={{ width: 42, height: 42, borderRadius: 11, background: color + '15', display: 'flex', alignItems: 'center', justifyContent: 'center', color, flexShrink: 0 }}>
        {icon}
      </div>
    ...
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 10, color: '#94a3b8', background: C.bg, borderRadius: 6, padding: '1px 6px', minWidth: 32, textAlign: 'center' }}>{Math.round(p)}%</span>
        </div>
      </div>
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (r.ok) { const t = await r.text(); if (t) setData(JSON.parse(t)) }
      setUpdated(new Date())
  ...
  const pieFlotte = data?.depenses?.flotte_by_type
    ? Object.entries(data.depenses.flotte_by_type).filter(([, v]) => v > 0)
        .map(([name, value], i) => ({ name, value, color: CAT_COLORS[i % CAT_COLORS.length] }))
        .sort((a, b) => b.value - a.value)
    : []

  const PERIOD_LABELS: Record<string, string> = { month: 'Ce mois', quarter: 'Ce trimestre', year: 'Cette année' }
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 400 }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: 40, height: 40, border: `3px solid ${C.blue}`, borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 14px' }} />
    ...
    <div style={{ padding: '24px 28px', maxWidth: 1700, margin: '0 auto', background: C.bg, minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 26, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ width: 46, height: 46, borderRadius: 14, background: `linear-gradient(135deg,${C.primary},${C.blue})`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', boxShadow: `0 4px 14px ${C.blue}40` }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>{PERIOD_LABELS[period]} · Mis à jour : {updated.toLocaleTimeString('fr-BE')}</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ display: 'flex', background: '#fff', border: `1.5px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
            {(['month', 'quarter', 'year'] as const).map((p, i) => (
              <button key={p} onClick={() => setPeriod(p)} className="tab-btn" style={{ padding: '8px 18px', border: 'none', borderRight: i < 2 ? `1px solid ${C.border}` : 'none', background: period === p ? C.blue : 'transparent', color: period === p ? '#fff' : C.slate, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                {['Mois', 'Trimestre', 'Année'][i]}
              </button>
            ))}
          </div>
          <button onClick={load} style={{ padding: '9px 12px', background: '#fff', border: `1.5px solid ${C.border}`, borderRadius: 11, cursor: 'pointer', color: C.slate, display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600 }}>
      ...
      <div className="db-card" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 14, marginBottom: 16 }}>
        <KpiCard label="Chiffre d'affaires encaissé" value={periodRevenue} sub={`${data?.invoices.paid || 0} facture(s) payée(s) · ${PERIOD_LABELS[period].toLowerCase()}`} color={C.green}
        ...
        <KpiCard label="Dépenses totales" value={periodDepenses} sub={`Fournisseurs + RH + Flotte · ${PERIOD_LABELS[period].toLowerCase()}`} color={C.red}
        ...
        <KpiCard label="Résultat net" value={periodResultat} sub={`Revenus − Dépenses · ${PERIOD_LABELS[period].toLowerCase()}`} color={periodResultat >= 0 ? C.green : C.red} alert={periodResultat < 0}
        ...
        <KpiCard label="En attente d'encaissement" value={data?.revenue.pending || 0} sub={`${data?.invoices.pending || 0} facture(s) non réglée(s)`} color={C.amber}
      ...
      {alerts.length > 0 && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          {alerts.map((a, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px', background: a.level === 'error' ? '#fef2f2' : '#fffbeb', border: `1.5px solid ${a.level === 'error' ? '#fecaca' : '#fde68a'}`, borderRadius: 10, fontSize: 12, color: a.level === 'error' ? '#991b1b' : '#92400e', fontWeight: 500 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 14, marginBottom: 14 }}>
        ...
        {/* Graphique revenus vs dépenses ventilées */}
        <div className="db-card" style={{ background: '#fff', borderRadius: 16, border: `1px solid ${C.border}`, padding: 22, boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 8 }}>
            <div>
              ...
              <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>Détail par catégorie · {period === 'month' ? 'Mois en cours' : period === 'quarter' ? '3 derniers mois' : '12 derniers mois'}</p>
            </div>
        ...
        <div className="db-card" style={{ background: '#fff', borderRadius: 16, border: `1px solid ${C.border}`, padding: 22, boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ borderTop: `2px solid ${C.border}`, paddingTop: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 20, fontWeight: 900, color: periodResultat >= 0 ? C.green : C.red }}>{fmt(periodResultat)}</span>
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 11, overflow: 'hidden' }}>
            {([
              ...
              <button key={t.key} onClick={() => setDepTab(t.key)} className="tab-btn" style={{ padding: '7px 16px', border: 'none', borderRight: i < arr.length - 1 ? `1px solid ${C.border}` : 'none', background: depTab === t.key ? C.primary : 'transparent', color: depTab === t.key ? '#fff' : C.slate, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                {t.label}
              </button>
            ))}
          </div>
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }}>
                {[
                  ...
                  { l: 'Véhicules actifs',         v: data?.fleet?.active || 0,           c: C.blue },
      ...
      {/* ── Stats opérationnelles + Résultat net + Actions ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(155px,1fr))', gap: 10, marginBottom: 14 }}>
        <StatCard label="Clients actifs"   value={data?.clients?.active || 0}    sub={`sur ${data?.clients?.total_clients || 0} total`}              color={C.blue}   icon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>} />
        <StatCard label="Projets actifs"   value={data?.projects?.active || 0}   sub={`sur ${data?.projects?.total || 0} total`}                     color={C.purple} icon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/></svg>} />
        <StatCard label="Employés actifs"  value={data?.employees?.active || 0}  sub={`Masse : ${fmt(data?.rh?.masse_salariale || 0)}/mois`}          color={C.amber}  icon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>} />
        <StatCard label="Véhicules actifs" value={data?.fleet?.active || 0}       sub={`sur ${data?.fleet?.total || 0} total`}                        color={C.cyan}   icon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>} />
        <StatCard label="Articles stock"   value={data?.stock?.total || 0}         sub={`Val. coût : ${fmt(data?.stock?.total_value || 0)}`}            color={C.teal}   icon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>} />
        <StatCard label="Stock critique"   value={data?.stock?.low_stock || 0}     sub={`${data?.stock?.out_of_stock || 0} en rupture totale`}           color={data?.stock?.low_stock ? C.red : C.green} icon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>} />
        <StatCard label="Factures en retard" value={data?.invoices?.overdue || 0} sub="Paiements clients en attente"                                   color={data?.invoices?.overdue ? C.red : C.green} icon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>} />
        <StatCard label="Fournisseurs"     value={fmt(data?.depenses?.fournisseurs_total || 0)} sub={`${data?.external_invoices?.total || 0} factures`} color={C.primary} icon={<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>} />
      </div>
        ...
        <div className="db-card" style={{ background: '#fff', borderRadius: 16, border: `1px solid ${C.border}`, padding: 22, boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}>
            {[
              ...
              <a key={i} href={q.href} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, background: q.color + '0e', border: `1.5px solid ${q.color}28`, textDecoration: 'none', transition: 'all 0.15s' }}
                onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = q.color + '18'; (e.currentTarget as HTMLAnchorElement).style.transform = 'translateY(-1px)' }}
                onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = q.color + '0e'; (e.currentTarget as HTMLAnchorElement).style.transform = '' }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: q.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <svg width="12" height="12" fill="none" stroke="#fff" strokeWidth="2.5" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {alerts.slice(0, 3).map((a, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 10px', background: a.level === 'error' ? '#fef2f2' : '#fffbeb', borderRadius: 8, fontSize: 11, color: a.level === 'error' ? '#991b1b' : '#92400e' }}>
                    <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                    {a.msg}
                  </div>
                ))}
              </div>
