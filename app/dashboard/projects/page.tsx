...
function ProgressBar({value,color,height=8}:{value:number;color:string;height?:number}) {
  return (
    <div style={{width:'100%',height,background:'#f1f5f9',borderRadius:height,overflow:'hidden'}}>
      <div style={{height:'100%',width:`${Math.min(value,100)}%`,background:color,borderRadius:height,transition:'width 0.4s cubic-bezier(0.4,0,0.2,1)'}}/>
    </div>
  )
}
          ...
          {tab==='info'&&(
            <div style={{padding:24,display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
              ...
              <div style={{gridColumn:'1/-1'}}>
                ...
                <input type="range" min="0" max="100" value={form.progress} onChange={e=>f('progress',+e.target.value)} style={{width:'100%',accentColor:'#7c3aed',height:6,cursor:'pointer'}}/>
        ...
        <div style={{flex:1,overflowY:'auto',padding:18,display:'flex',flexDirection:'column',gap:14}}>
          <div style={{background:'#f8fafc',borderRadius:14,padding:16,border:'1px solid #f1f5f9'}}>
            ...
            <div style={{marginTop:12}}>
              <input type="range" min="0" max="100" value={prog} onChange={e=>setProg(+e.target.value)} style={{width:'100%',accentColor:'#7c3aed',cursor:'pointer'}}/>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {Object.entries(STATUS).filter(([k])=>k!==project.status).map(([k,v])=>(
                <button key={k} onClick={()=>onStatusChange(project.id,k as Project['status'])}
                  style={{padding:'5px 12px',borderRadius:20,border:`1px solid ${v.dot}40`,background:v.bg,color:v.color,fontSize:11,fontWeight:700,cursor:'pointer'}}>
                  {v.label}
                </button>
              ))}
            </div>
...
export default function ProjectsPage() {
  ...
  async function del(id: string) {
    const uid = await getUserId()
    await fetch(`/api/projects?id=${id}`, { method:'DELETE', headers: { 'x-user-id': uid ?? '' } })
    setDeleteId(null); load()
  }

  async function changeStatus(id: string, status: Project['status']) {
    const uid = await getUserId()
    await fetch('/api/projects', { method:'PATCH', headers: { 'Content-Type':'application/json', 'x-user-id': uid ?? '' }, body: JSON.stringify({ id, status }) })
    load()
  }

  async function changeProgress(id: string, progress: number) {
    const uid = await getUserId()
    await fetch('/api/projects', { method:'PATCH', headers: { 'Content-Type':'application/json', 'x-user-id': uid ?? '' }, body: JSON.stringify({ id, progress }) })
  ...
  const filtered = projects
    .filter(p => {
      ...
      return hit && (statusF==='all'||p.status===statusF) && (priorityF==='all'||p.priority===priorityF) && (clientF==='all'||p.client_id===clientF)
    })
    .sort((a,b)=>{
  ...
  const kpi = {
    ...
    overdue:   projects.filter(p=>p.status!=='completed'&&p.status!=='cancelled'&&p.end_date&&(daysLeft(p.end_date)??0)<0).length,
  ...
  return (
    <div style={{padding:24,maxWidth:1600,margin:'0 auto'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:12,marginBottom:18}}>
        {[
          ...
          <div key={i} style={{background:k.bg,borderRadius:13,border:`1.5px solid ${k.border}`,padding:'12px 14px'}}>
            ...
            <p style={{fontSize:(k as any).isText?13:20,fontWeight:800,color:k.color}}>{k.v}</p>
          </div>
        ))}
      </div>
                  ...
                  <tr key={p.id} className="rh" style={{borderBottom:'1px solid #f8fafc',cursor:'pointer'}} onClick={()=>setViewP(p)}>
                    ...
                    <td style={{padding:'12px 14px'}} onClick={e=>e.stopPropagation()}>
                      <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:4}}>
                        <button onClick={()=>setViewP(p)} style={{height:30,width:30,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:8,border:'none',cursor:'pointer',background:'#f8fafc',color:'#94a3b8'}}>{I.eye}</button>
                        <button onClick={()=>openEdit(p)} style={{height:30,width:30,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:8,border:'none',cursor:'pointer',background:'#f8fafc',color:'#94a3b8'}}>{I.edit}</button>
                        <button onClick={()=>setDeleteId(p.id)} style={{height:30,width:30,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:8,border:'none',cursor:'pointer',background:'#f8fafc',color:'#94a3b8'}}>{I.trash}</button>
                      </div>
                    </td>
                  </tr>
                )
                ...
                <div style={{padding:'16px 16px 14px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                    <div style={{flex:1,marginRight:10}}>
                      ...
                      {p.description&&<p style={{fontSize:11,color:'#94a3b8',marginTop:4,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical' as const}}>{p.description}</p>}
                    </div>
                  ...
                  <div style={{display:'flex',gap:6,paddingTop:10,borderTop:'1px solid #f1f5f9'}} onClick={e=>e.stopPropagation()}>
                    <button onClick={()=>setViewP(p)} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:5,padding:'8px 0',borderRadius:8,border:'none',background:'#f8fafc',fontSize:12,fontWeight:600,color:'#64748b',cursor:'pointer'}}>{I.eye} Voir</button>
                    <button onClick={()=>openEdit(p)} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:5,padding:'8px 0',borderRadius:8,border:'none',background:'#f8fafc',fontSize:12,fontWeight:600,color:'#64748b',cursor:'pointer'}}>{I.edit} Modifier</button>
                    <button onClick={()=>setDeleteId(p.id)} style={{width:34,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:8,border:'none',background:'#f8fafc',color:'#94a3b8',cursor:'pointer'}}>{I.trash}</button>
                  </div>
                </div>
              </div>
            )
      ...
      <ProjectModal open={modal} onClose={()=>{setModal(false);setEditP(null)}} onSave={save} initial={editP} clients={clients}/>
      <ProjectDrawer project={viewP} clients={clients} onClose={()=>setViewP(null)} onEdit={()=>{ if(viewP) openEdit(viewP) }} onDelete={()=>{ if(viewP) openDelete(viewP) }} onStatusChange={changeStatus} onProgressChange={changeProgress}/>
    </div>
  )
}
